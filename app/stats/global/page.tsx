...
export default function GlobalLeaderboard() {
  ...
  useEffect(() => {
    fetch('/api/public/stats/global', { cache: 'no-store' })
      .then(res => res.json())
      .then(data => { 
        setStats(data); 
        setLoading(false); 
  ...
  const requestSort = (key: string) => {
    let direction = 'desc';
    // ERA and WHIP are better when lower, so we can flip the default if you prefer, 
    // but standard sort logic works:
  ...
  if (loading) return <div className="min-h-screen bg-[#fdf0d5] flex items-center justify-center font-black uppercase text-[#001d3d] animate-bounce italic text-4xl">LOADING STATS...</div>;
              ...
              {displayData.map((p, i) => (
                <tr key={i} className="hover:bg-[#ffd60a]/10 transition-colors group">
                  ...
                  {viewMode === 'pitchers' ? (
                    ...
                    <>
                      {/* ... Hitting rows exactly as before ... */}
