...
export async function GET(request: Request) {
  try {
    ...
    // Using UTC ensures Prisma doesn't accidentally cut off Jan 1st or Dec 31st based on your timezone
    ...
    if (seasonIdFilter) {
      // PATH A: Specific Season (Used by your new League Stats Hub)
      whereClause.game = { seasonId: parseInt(seasonIdFilter) };
    } else {
      // PATH B: Current Year Default (Used by the Global Stats page)
    ...
    atBats.forEach((ab) => {
      ...
      // Optional chaining protects against orphaned test data crashing the loop
      const leagueName = ab.game?.season?.league?.name || 'Unknown League';
      if (seasonIdFilter) seasonName = ab.game?.season?.name || 'Season Stats';
    ...
    const batters = Object.values(batterMap).map((b: any) => {
      ...
      return {
        ...
        obp: obp.toFixed(3).replace(/^0/, ''), // <-- THIS WAS MISSING!
    ...
    return NextResponse.json({ batters, pitchers, year: currentYear, seasonName });
  } catch (error: any) {
