...
export default function CircuitWizard() {
  ...
  const handleCreate = async () => {
    if (!rules.name) return alert("Please name this Circuit!");
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-[#fdf0d5] text-[#001d3d] border-[16px] border-[#001d3d] p-8 md:p-12">
      <div className="max-w-4xl mx-auto">
        ...
        <header className="mb-12 border-b-8 border-[#c1121f] pb-6 flex justify-between items-end">
          <div>
            <h1 className="text-6xl font-black italic uppercase tracking-tighter text-[#001d3d] drop-shadow-[4px_4px_0px_#ffd60a]">Circuit Wizard</h1>
            <p className="text-[#c1121f] font-bold uppercase text-[10px] tracking-[0.4em] mt-2 italic">WIFF+ // Multi-Event Tour Setup</p>
          </div>
        ...
        <div className="bg-white border-4 border-[#001d3d] p-8 shadow-[12px_12px_0px_#c1121f] space-y-10">
          ...
          <section>
            <label className="block text-[10px] font-black uppercase text-[#669bbc] mb-2 tracking-widest">Circuit Name</label>
            <input 
              ...
              placeholder="e.g. 2026 SUMMER TOUR"
          ...
          >
            {loading ? 'ESTABLISHING CIRCUIT...' : 'Launch Circuit'}
          </button>
        </div>
      </div>
    </div>
