...
export default function MasterPlayerRegistry() {
  ...
  const handleUpdateName = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || "Name update failed.");
      }
    } catch (e) {
  ...
  const handleDeletePlayer = async (id: number, name: string) => {
    ...
    try {
      ...
      alert("Network error while trying to delete player.");
    }
  };
            ...
            filteredPlayers.map(player => {
              ...
              return (
                ...
                >
                  ...
                  {!isMergeMode && editingId !== player.id && canManage && (
                    <div className="flex gap-2">
                      <button 
                        onClick={() => { setEditingId(player.id); setEditName(player.name); }}
                        className="bg-[#c1121f] text-white px-4 py-2 text-[10px] font-black uppercase italic transition-all hover:bg-[#ffd60a] hover:text-[#001d3d] shadow-[4px_4px_0px_#001d3d] active:translate-y-1 active:shadow-none"
                      >
                        Edit
                      </button>
                      <button 
                        onClick={() => handleDeletePlayer(player.id, player.name)}
                        className="bg-white border-2 border-slate-300 text-slate-400 px-3 py-2 text-[10px] font-black uppercase italic transition-all hover:bg-red-600 hover:text-white hover:border-red-600 shadow-[4px_4px_0px_#001d3d] active:translate-y-1 active:shadow-none"
                      >
                        Delete
                      </button>
                    </div>
